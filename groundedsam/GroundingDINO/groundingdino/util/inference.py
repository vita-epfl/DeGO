...
def load_image(image_path: str, random_resize=[800]) -> Tuple[np.array, torch.Tensor]:
    transform = T.Compose(
        [
            T.RandomResize(random_resize, max_size=1333),
            T.ToTensor(),
...
def predict(
    ...
    if not only_max_prompt:
        ...
        max_logit_bool = torch.zeros_like(logits, dtype=torch.bool)
        max_logit_bool[torch.arange(logits.size(0)), max_logits] = True
        phrases = [
            get_phrases_from_posmap(logit, tokenized, tokenizer).replace('.', '')
            for logit
            in max_logit_bool
        ]
