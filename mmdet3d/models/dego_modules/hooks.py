...
from mmcv.runner.hooks import HOOKS, CosineAnnealingLrUpdaterHook, Hook
...
@HOOKS.register_module()
class DeGOSetEpochInfoHook(Hook):
    """Expose the runner epoch to models that gate losses by epoch.

    MMCV detectors do not receive the runner in ``forward_train``. This hook
    keeps the model informed with a 1-based epoch number, matching log output
    such as ``Epoch [21]``.
    """
    ...
    def before_train_epoch(self, runner: 'runner.BaseRunner') -> None:
        ...
        if hasattr(model, 'set_epoch'):
            model.set_epoch(runner.epoch + 1, runner.iter)
