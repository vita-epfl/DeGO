...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        input_dict['has_prev_sample'] = index > 0 and self.data_infos[index - 1]['scene_token'] == info['scene_token']
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                # Also load info for the temporal render frames
                if self.temporal_frame_ids is not None:
                    info_temporal_render = self.get_temporal_render_info(info, index)
                    input_dict.update(dict(render_frames=info_temporal_render))

        return input_dict
    
    def get_temporal_render_info(self, info, index):
        temporal_list = []
        for t in self.temporal_frame_ids:
            select_id = index + t
            # select_id = min(max(index + t, 0), len(self.data_infos)-1)
            if select_id<0 or select_id>=len(self.data_infos):
                temp_info = info
                temp_info['valid'] = False
            elif self.data_infos[select_id]['scene_token'] != info['scene_token']:
                temp_info = info
                temp_info['valid'] = False
            else:
                # Get all needed info here:
                temp_info = self.data_infos[select_id]
                temp_info['valid'] = True

            # Get all needed info here:
            temporal_list.append({
                    k:v for k, v in temp_info.items() if k in 
                    ['token','lidar_idx', 'lidar_path', 'lidar2ego_translation', 'lidar2ego_rotation', 
                     'ego2global_translation', 'ego2global_rotation', 'cams', 'panoptic_path', 'valid']
            })
    ...
    def get_adj_info(self, info, index):
        ...
        for select_id in adj_id_list:
            select_id_t = max(index - select_id, 0)
            while not self.data_infos[select_id_t]['scene_token'] == info['scene_token']:
                select_id_t += 1
                assert select_id_t <= index, 'No valid adjacent frame found'
            info_adj_list.append(self.data_infos[select_id_t])
